...
class BartTorvik(BaseClient):
    ...
    def _parse_csv(self, csv_text: str) -> pd.DataFrame:
        ...
        prepared_csv = self._prepare_csv_text(csv_text)
        if not prepared_csv:
            return pd.DataFrame()
        ...
        for parse_kwargs in parse_attempts:
            try:
                ...
                if len(df.columns) <= 1:
                    last_error = ValueError("Parsed single-column response instead of tabular CSV")
                    continue
                return df
            except Exception as e:
                last_error = e
        ...
        if last_error:
            self.log.warning(f"CSV parse error: {last_error}")
        return pd.DataFrame()

    @staticmethod
    def _prepare_csv_text(csv_text: str) -> str:
        """Trim BOM/preamble lines so pandas sees the real CSV header first."""
        ...
        header_tokens = (
            "team", "conf", "rk", "rank", "adjoe", "adjde",
            "barthag", "date", "opp", "rec", "g",
        )
        header_index = 0

        for idx, line in enumerate(lines[:25]):
            lower = line.lower()
            delimiter_count = max(line.count(","), line.count("\t"), line.count(";"))
            token_hits = sum(token in lower for token in header_tokens)
            if delimiter_count >= 1 and token_hits >= 2:
                header_index = idx
                break
