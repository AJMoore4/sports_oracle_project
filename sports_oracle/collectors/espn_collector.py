...
class ESPNCollector(BaseClient):
    ...
    def get_recent_scoreboard(self, days_back: int = 10) -> pd.DataFrame:
        ...
        combined = self._concat_scoreboards(all_games)
    ...
    @staticmethod
    def _concat_scoreboards(frames: list[pd.DataFrame]) -> pd.DataFrame:
        """Drop empty/all-NA entries before concatenation to keep pandas quiet."""
        cleaned_frames = []
        for frame in frames:
