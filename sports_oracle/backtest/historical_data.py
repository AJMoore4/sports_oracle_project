...
class HistoricalDataBuilder:
    ...
    def _simulate_game(
        ...
        h_close_rate = round(float(np.clip(rng.normal(0.28, 0.09), 0.05, 0.65)), 3)
        a_close_rate = round(float(np.clip(rng.normal(0.28, 0.09), 0.05, 0.65)), 3)
        h_close_games = int(np.clip(round(rng.normal(31, 2) * h_close_rate), 1, 20))
        a_close_games = int(np.clip(round(rng.normal(31, 2) * a_close_rate), 1, 20))
        home_extended = {
            "close_game_pct": round(float(np.clip(rng.normal(0.50, 0.15), 0.0, 1.0)), 3),
            "close_game_rate": h_close_rate,
            "close_game_games": h_close_games,
        ...
        away_extended = {
            "close_game_pct": round(float(np.clip(rng.normal(0.50, 0.15), 0.0, 1.0)), 3),
            "close_game_rate": a_close_rate,
            "close_game_games": a_close_games,
            "margin_std": round(float(np.clip(rng.normal(12.0, 3.0), 4.0, 25.0)), 2),
            "conf_strength": round(float(np.clip(
        ...
        row = {
            ...
            "h_close_rate": home_extended.get("close_game_rate", 0.25),
            "a_close_rate": away_extended.get("close_game_rate", 0.25),
            "h_close_games": home_extended.get("close_game_games", 0),
            "a_close_games": away_extended.get("close_game_games", 0),
            ...
            "venue_sample_size": inputs["venue"].get("sample_size", 0),
            ...
            "market_total": None,
            "market_total_available": 0,
            "market_total_delta": 0.0,
        }
    ...
    def get_feature_columns() -> list[str]:
        ...
        return [
            # Formula outputs
            "formula_margin", "formula_total", "raw_total", "formula_win_prob", "game_pace",

            # Layer 4 adjustments
            "adj_momentum", "adj_experience", "adj_rest", "adj_injury",
            "adj_seed", "adj_travel", "adj_total", "adj_total_points",
            ...
            "h_close_rate", "a_close_rate",
            "h_close_games", "a_close_games",
            ...
            "vsi", "vpi", "v3p", "venue_sample_size",

            # Total calibration context
            "market_total", "market_total_available", "market_total_delta",
