...
if __name__ == "__main__":
    ...
    print("\n📈 Evaluator — Backtesting the Machine Learning Engine")
    print("=" * 60)
    ...
    print("  🤖 Training ML Model with new parameters...")
    ml = MLPredictor(blend_weight=0.45)
    ml.train(df)

    # 2. GENERATE PREDICTIONS: Run the historical data through the trained ML model
    print("  🧠 Generating ML predictions for all historical games...")
    X = df[ml.feature_cols].fillna(0)
    X_scaled = ml.scaler.transform(X)

    df["ml_margin"] = ml.margin_model.predict(X_scaled)
    df["ml_total"] = ml.total_model.predict(X_scaled)
    df["ml_win_prob"] = ml.win_model.predict_proba(X_scaled)[:, 1]

    # 3. Grade the new ML columns!
    evaluator = Evaluator()
    report = evaluator.evaluate_from_training(
        df,
        prob_col="ml_win_prob",
        margin_col="ml_margin",
        total_col="ml_total"
    )
    print(report.summary())