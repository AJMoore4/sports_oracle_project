...
class SeedHistory:
    ...
    def get_win_rate(
        ...
        weighted_wins = 0.0
        total_weight = 0.0
        ref_year = self._resolve_reference_year(reference_year)
    ...
    def _resolve_reference_year(self, reference_year: Optional[int] = None) -> int:
        if reference_year is not None:
            return int(reference_year)
        if self.reference_year is not None:
            return int(self.reference_year)
        now = datetime.now()
