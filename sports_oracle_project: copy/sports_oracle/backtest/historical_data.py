...
APPROACH:
  ...
  2. Produce a flat DataFrame: one row per game, ~62 features
...
class HistoricalDataBuilder:
    ...
    def build_synthetic_training_set(
        ...
        This is NOT random noise — the outcomes correlate with
        efficiency gaps the same way real games do, with the
        right amount of variance (σ ≈ 10-11 points).
    ...
    def _generate_bracket(
        ...
        round1_winners = {r: [] for r in regions}

        for region in regions:
            for h_seed, l_seed in FIRST_ROUND_MATCHUPS:
                game_id += 1
                game = {
                    ...
                    "home_seed": h_seed,
                    "away_seed": l_seed,
                    "home_eff": self._draw_efficiency(h_seed, rng),
                    "away_eff": self._draw_efficiency(l_seed, rng),
                }
                games.append(game)

                # Determine winner for next round
                winner_seed, winner_eff = self._sim_winner(game, rng)
                round1_winners[region].append((winner_seed, winner_eff))

        # Second round: 4 regions × 4 games = 16 games
        round2_winners = {r: [] for r in regions}
        for region in regions:
            winners = round1_winners[region]
            for i in range(0, len(winners), 2):
                ...
                round2_winners[region].append((ws, we))

        # Sweet 16, Elite 8, Final Four, Championship
        prev_winners = round2_winners
        for rd, rd_name in [(3, "Sweet 16"), (4, "Elite 8")]:
            next_winners = {r: [] for r in regions}
            for region in regions:
                winners = prev_winners[region]
                for i in range(0, len(winners), 2):
                    if i + 1 >= len(winners):
                        break
                    game_id += 1
        ...
        # Championship (skip if bracket doesn't yield 2 FF winners)
        # In practice this always works but we're defensive
    ...
    def _simulate_game(
        ...
        # True margin = formula margin + noise (σ=11)
        actual_margin = result.spread * -1 + rng.normal(0, 11.0)
        # Round the scores realistically
